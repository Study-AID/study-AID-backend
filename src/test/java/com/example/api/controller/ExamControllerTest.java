...
public class ExamControllerTest {
    ...
    void toggleLikeExamItem_AddLike() throws Exception {
        ...
        ToggleLikeExamItemOutput output = new ToggleLikeExamItemOutput(
                examId,
                examItemId,
                userId,
                true
        ...
        when(examService.findExamById(examId)).thenReturn(Optional.of(testExamOutput));
        when(examService.toggleLikeExamItem(any(ToggleLikeExamItemInput.class))).thenReturn(output);

        // when, then
        mockMvc.perform(post("/v1/exams/{examId}/items/{examItemId}/like", examId, examItemId)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @DisplayName("시험 문제 좋아요 토글 - 권한 없음")
    @WithMockUser
    void toggleLikeExamItem_Forbbidden() throws Exception {
        // given
        UUID examItemId = UUID.randomUUID();
        UUID anotherUserId = UUID.randomUUID(); // 다른 사용자 ID
        ToggleLikeExamItemRequest request = new ToggleLikeExamItemRequest();

        // 다른 사용자의 시험로 설정
        testExamOutput.setUserId(anotherUserId);
